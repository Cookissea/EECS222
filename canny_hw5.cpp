...
#define THIGH 0.8
#define BUFSIZE 1
#define FIRSTFILE "video/EngPlaza000.pgm"
#define FIRSTOUTFILE "EngPlaza000_edges.pgm"
...
SC_MODULE(Simulus)
{
   sc_fifo_out<IMAGE> sp;//Simulus port
   char infilename[22];  /* Name of the input image */
   unsigned char image[SIZE];     /* The input image */
   ...
   void read_frame(){
      int frame_size = 20;
      strcpy(infilename,FIRSTFILE);
      for (int i = 1; i <= frame_size; ++i)
      {
         infilename[15] = i/10+'0';
         infilename[16] = i%10+'0';
...
SC_MODULE(DataIn)
{
   sc_fifo_in<IMAGE> dip1;//DataIn port 1
   sc_fifo_out<IMAGE> dip2;
   ...
   void data_in(){
      while(true)
         dip2.write(dip1.read());
   }
};

SC_MODULE(DUT)
{
   sc_fifo_in<IMAGE> dp1;//DUT port 1
   sc_fifo_out<IMAGE> dp2;
   unsigned char edge[SIZE];
   unsigned char *image;

   SC_CTOR(DUT){
      SC_THREAD(edge_detect);
      set_stack_size(128*1024*1024);
   }

   void edge_detect(){
      while(true){
         image = (unsigned char*)dp1.read();
         ...
         canny(image, ROWS, COLS, SIGMA, TLOW, THIGH, edge,FIRSTFILE);
         dp2.write(IMAGE(edge));
      }
...
SC_MODULE(DataOut)
{
   sc_fifo_in<IMAGE> dop1;//DataOut port 1
   sc_fifo_out<IMAGE> dop2;
   ...
   void data_out(){
      while(true)
         dop2.write(dop1.read());
   }
};   

SC_MODULE(Platform)
{
   sc_fifo_in<IMAGE> pp1;//Platform port 1
   sc_fifo_out<IMAGE> pp2;   
   sc_fifo<IMAGE> q1,q2;
   DataIn in;
   DUT canny;
   DataOut out;
   ...
   /*void function(){
         //Error within next two lines
         // q1 = new sc_fifo<IMAGE>("Q1");
         // q2 = new sc_fifo<IMAGE>("Q2");
         in.dip1.bind(pp1);
         in.dip2.bind(q1);
         canny.dp1.bind(q1);
         canny.dp2.bind(q2);
   ...
   SC_CTOR(Platform)
   :q1("Q1",BUFSIZE),q2("Q2",BUFSIZE),in("DataIn"),canny("canny"),out("DataOut")
   {
      in.dip1.bind(pp1);
      in.dip2.bind(q1);
      canny.dp1.bind(q1);
...
SC_MODULE(Monitor)
{
   char outfilename[27];    /* Name of the output "edge" image */
   sc_fifo_in<IMAGE> mp;
   ...
   void write_frame(){
      int frame_size = 20;
      strcpy(outfilename,FIRSTOUTFILE);
      for (int i = 1; i <= frame_size; ++i)
      {
         outfilename[9] = i/10+'0';
         outfilename[10] = i%10+'0';
         if(VERBOSE) printf("Writing the edge iname in the file %s.\n", outfilename);
         if(write_pgm_image(outfilename, (unsigned char*)mp.read(), ROWS, COLS, "", 255) == 0){
...
{  
   ...
   /*void top(){
         // f1 = new sc_fifo<IMAGE>("F1");
         // f2 = new sc_fifo<IMAGE>("F2");
         s.sp.bind(f1);
         p.pp1.bind(f1);
         p.pp2.bind(f2);
         m.mp.bind(f2);
   ...
   SC_CTOR(Top)
   :f1("F1",BUFSIZE),f2("F2",BUFSIZE),s("Simulus"),p("Platform"),m("Monitor")
   {
      s.sp.bind(f1);
      p.pp1.bind(f1);
      p.pp2.bind(f2);
         ...
         float tlow, float thigh, unsigned char *edge, const char *fname)
{
   ...
   unsigned char nms[SIZE] = {0};        /* Points that are local maximal magnitude. */
   short int smoothedim[SIZE] = {0},     /* The image after gaussian smoothing.      */
             delta_x[SIZE] = {0},        /* The first devivative image, x-direction. */
             delta_y[SIZE] = {0},        /* The first derivative image, y-direction. */
             magnitude[SIZE] = {0};      /* The magnitude of the gadient image.      */
   //int r, c, pos;
   //float dir_radians[SIZE];   /* Gradient direction image.                */
...
* DATE: 2/15/96
*******************************************************************************/
void DUT::derrivative_x_y(short int *smoothedim, int rows, int cols,
        short int *delta_x, short int *delta_y)
{
   int r, c, pos;
        ...
        short int *smoothedim)
{
   ...
   float tempim[SIZE],        /* Buffer for separable filter gaussian smoothing. */
         kernel[21],        /* A one dimensional gaussian kernel. */
...
* DATE: 2/15/96
*******************************************************************************/
void DUT::non_max_supp(short *mag, short *gradx, short *grady, int nrows, int ncols,
    unsigned char *result) 
{
...
******************************************************************************/
int Simulus::read_pgm_image(char *infilename, unsigned char *image, int rows,
    int cols)
{
   FILE *fp;
   char buf[71];
